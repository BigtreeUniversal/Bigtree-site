...
// 🕒 Horloge SynchroLink centrale intégrée

let CLK = Math.floor(Date.now() / 1000);  // point de départ du timecode
let latestClockTrame = `CLK:${CLK}|SRC:CLOCK|`;

// Mise à jour toutes les 500 ms
setInterval(() => {
  CLK++;
  latestClockTrame = `CLK:${CLK}|SRC:CLOCK|`;
}, 500);

// Route pour exposer la trame clock en temps réel
app.get('/clock/latest', (req, res) => {
  res.json({ trame: latestClockTrame });
});
